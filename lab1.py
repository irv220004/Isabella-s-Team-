...
##IMPORTING DATETIME LIBRARY FOR DATE INFORMATION##
from datetime import datetime                                                       #adds datetime functionality to the program for accurate case length information 
...
def clean_record(record):                                                           #parameter added to allow arguments to be passed to the function from the file input section
    ...
    years_unsolved = datetime.now().year - year                                    #defines years unsolved
...
try: 
    ...
    ages = [row[2] for row in records]                                                                                           #age list created by iterating through the records list and selecting the age field (index 2) from each record
    average_age = sum(ages) / len(ages)                                                                                          #average age calculated by summing values in the list and dividing by list length 
